...
class tags(commands.Cog):
    ...
    async def _tag(self, ctx, *, name):
        async with self.bot.pool.acquire() as conn:
            tag = await conn.fetchrow(
                "SELECT * FROM tags WHERE name = $1 AND guild = $2",
                name.lower(),
            ...
            if not tag:
                return await ctx.channel.send(
                    "That isn't a tag. Don't know what you're thinking of."
                )
            ...
            try:
                ...
                await conn.execute(
                    "UPDATE tags SET uses = uses + $1 WHERE id = $2 AND guild = $3",
                    1,
                    tag["id"],
                    tag["guild"],
                )
    ...
    async def _tag_create(self, ctx, name, *, content):
        if len(name) > 30:
            return await ctx.channel.send(
                "Nope. Not gonna do it. No, no no. (Name too long)"
            )
        if len(content) > 2000:
            return await ctx.channel.send(
                "Nope. Not gonna do it. No, no no. (Content too long)"
            )
        ...
        async with self.bot.pool.acquire() as conn:
            ...
            if tag:
                return await ctx.channel.send(
                    "No go my guy... That tag already exists!"
                )

            try:
                await conn.fetchrow(
                    f"INSERT INTO tags(created,name,content,guild,author) VALUES($1,$2,$3,$4,$5) RETURNING *",
                    datetime.utcnow(),
                    name.lower(),
                    content,
                    ctx.channel.guild.id,
    ...
    async def _tag_delete(self, ctx, *, name):
        async with self.bot.pool.acquire() as conn:
            ...
            try:
                await conn.fetchrow(
                    "DELETE FROM tags WHERE id = $1 AND guild = $2",
                    tag["id"],
                    ctx.channel.guild.id,
                )
            except Exception as err:
            ...
            await ctx.channel.send(
                f":ok_hand: I deleted a tag with the name {tag['name']}"
            )
    ...
    async def _tag_stats(self, ctx, *, name):
        async with self.bot.pool.acquire() as conn:
            ...
            embed = discord.Embed(
                color=0x99AAB5, description=f"**Response**:\n{tag['content']}"
            )
            ...
            embed.add_field(
                name="Created",
                value=f"{arrow.get(tag['created']).humanize()} ({tag['created']})",
                inline=False,
            )
            try:
                ...
                return await ctx.channel.send(
                    f"Something went wrong :thonk: (A different message from always huh)\n{err}"
                )
