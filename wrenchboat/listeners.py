        ...
        if self.bot.automod[message.channel.guild.id]["token_remover"]:
            ...
            if check:
                try:
                    ...
                    embed = discord.Embed(
                        description=f":warning: I found a token in [this message]({message.jump_url}). You know what that means, it's been sent to [This gist]({data['url']}) for invalidation."
                    )
