...
in_line_responses = {
    ...
    "simple": {"en": "Done.", "es": "Hecho.", "la": "Factum.",},
...
class Responder:
    ...
    async def in_line(cls, ctx, user, action: str, reason, time="Indefinite"):
        ...
        returned_message = in_line_responses[ctx.bot.guild_responses[ctx.guild.id]][
            ctx.bot.language[ctx.channel.guild.id]
        ]
        try:
            await ctx.channel.send(
                returned_message.format(
                    guild=ctx.guild,
